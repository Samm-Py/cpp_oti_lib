...
\begin{abstract}
Automatic differentiation is a useful tool for obtaining sensitivities, but
introducing derivative calculations into existing C++ codes can require
substantial source modifications. This paper
presents \texttt{cpp\_oti\_lib}, a header-only C++ implementation of
order-truncated imaginary (OTI) numbers designed to make automatic
differentiation easier to introduce into existing C++ codes. An OTI algebra
\(\mathbb{OTI}_m^n\) contains \(m\) independent imaginary directions and
retains all terms through truncation order \(n\); therefore, one overloaded
evaluation returns the value of each computed output component together with
derivative information with respect to the \(m\) selected variables through
total order \(n\). The library represents this algebra with the C++ type
\texttt{oti::otinum<M,N>} and uses ordinary operator overloading for
\(+\), \(-\), \(\times\), and \(\div\), together with
\texttt{oti} namespace overloads of elementary functions such as \(\sin\),
\(\exp\), and \(\log\). As a result, AD can be introduced into many
existing simulation codes by including the header and changing selected scalar
types. The implementation includes a standard CPU OTI scalar type and a
Kokkos-compatible variant designed for execution in Kokkos kernels on supported
CPU and GPU backends. The Kokkos version replaces fixed coefficient storage
with \texttt{Kokkos::Array}, marks arithmetic and elementary operations with
\texttt{KOKKOS\_FUNCTION}, and is verified with tests that execute on
Kokkos-supported backends, lifting OTI-based automatic differentiation to
GPU-capable workflows.
\end{abstract}
...
by computer programs \cite{Griewank2008,Baydin2015,Bucker2005}. Unlike
symbolic differentiation, AD does not attempt to produce a closed-form
expression for a derivative. Unlike finite differences, AD does not approximate
derivatives by subtracting nearby function values. Instead, AD applies the
chain rule systematically to the elementary operations and intrinsic functions
that make up a program.

For a program written as a composition of elementary operations,
\[
    y = f(x),
\]
AD augments the computation so that derivative information is propagated along
with the primal value. If an intermediate variable is computed as
\[
    v = g(u),
\]
then its derivative is propagated through
\[
...
The scalar OTI implementation in this library is most closely aligned with
forward-mode AD. Each OTI variable carries not only a real value, but also a
structured collection of derivative coefficients. When arithmetic and
elementary functions are evaluated with OTI inputs, the overloaded C++
operators propagate all stored coefficients through the computation. The final
OTI value therefore contains the real function value and derivatives with
respect to the selected variables up to the configured total order.

The key difference between ordinary first-order forward-mode AD and the static
OTI representation is that OTI stores a full truncated multivariate Taylor
expansion. This allows gradients, Hessian entries, mixed derivatives, and
higher-order sensitivities to be recovered from one overloaded evaluation, so
long as the required derivative order is less than or equal to the template
parameter \(N\). This places the library near higher-order forward-mode AD and
Taylor-coefficient methods, but with a static OTI coefficient layout tailored
to scalar multivariable derivative computation.
...
derivative:
\begin{itemize}[noitemsep]
    \item \texttt{deriv(alpha)} returns \(c_\alpha\).
...
\texttt{deriv(alpha)} & Normalized coefficient for a multi-index \\
...
\begin{itemize}[noitemsep]
    ...
    \item \texttt{real()}, \texttt{data()}, \texttt{deriv(alpha)}, and
    \texttt{partial(alpha)}.
...
The benchmark suite evaluates the cost of OTI arithmetic as a function of the
number of directions \(m\), truncation order \(n\), and execution backend. The
primary comparison is between standard scalar evaluation, CPU OTI evaluation,
Kokkos CPU-backend OTI evaluation, and Kokkos GPU-backend OTI evaluation. The
benchmarks use representative scalar kernels built from polynomial,
trigonometric, exponential, logarithmic, and mixed operations so that both
coefficient convolution and elementary Taylor composition are exercised.

For each benchmark kernel, the measured quantities include wall-clock runtime,
effective throughput, speedup relative to the CPU OTI baseline, and derivative
agreement with the focused unit-test reference values. This organization
separates correctness from performance: the unit tests establish that the
computed coefficients are correct, while the benchmark suite quantifies how
the implementation scales across algebra sizes and hardware backends.

\subsection{CPU and GPU Results}

The CPU results establish the baseline cost of replacing a scalar
\texttt{double} evaluation with \texttt{oti::otinum<M,N>} evaluation. The
Kokkos CPU-backend results measure the overhead of the performance-portable
execution path on host hardware. The Kokkos GPU-backend results quantify the
benefit of evaluating many independent OTI computations in parallel on an
accelerator.

The expected performance trend is that a single OTI evaluation is more
expensive than a scalar evaluation because it propagates all coefficients up
to the selected order. However, when many independent evaluations are required,
as in parameter studies, design-space sampling, finite element quadrature
loops, or uncertainty quantification workflows, the Kokkos GPU backend exposes
parallelism across evaluations and reduces time-to-solution.
