...
// works for jets produced on the GPU, via the host-staging path.
//
// Pipeline per rank: evaluate f(x,y)=sin(x)*exp(y) over its slice of a 1000x1000
// grid on the DEVICE (Kokkos::View<Jet*> in CUDA space), deep_copy to a host
// mirror, MPI_Gatherv the host buffer to rank 0 using oti::mpi::make_datatype.
//
// Why host staging: on this box MPI (Intel MPI) is not CUDA-aware for the NVIDIA
// device, so we stage through a host mirror. The committed datatype itself is
// GPU-agnostic -- otinum's layout is identical in host and device memory (checked
// below) -- so a CUDA-aware build would instead pass the device pointer directly,
// with no change to the datatype.
//
// Verification is device-vs-device: rank 0 recomputes the whole grid on its GPU
// and memcmps against the gathered buffer. Both sides run the SAME kernel on the
// SAME hardware, so they are bit-identical even though sin/exp on the GPU need
// not match the CPU bit-for-bit. This isolates the MPI transport, which is what
// step 2 is actually testing.
//
...
int main(int argc, char** argv)
{
    ...
    Kokkos::initialize(argc, argv);
    {
        ...
        if (rank == 0) {
            ...
            std::printf("ranks            : %d (host-staged gather)\n", size);
